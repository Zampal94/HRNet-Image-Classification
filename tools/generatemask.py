...
def predict_image(image):
    ...
    #model=torch.load('/home/janischl/ssn-pytorch/classify/super_balanced.pth')
...
def inference(image, nspix, n_iter, fdim=None, color_scale=0.26, pos_scale=2.5, weight=None, enforce_connectivity=True):
    ...
    if weight is not None:
        ...
        modelssn = SSNModel(fdim, nspix, n_iter).to("cuda")
        checkpoint = torch.load(weight)
        modelssn.load_state_dict(checkpoint)#['model_state_dict']
        modelssn.eval()
    else:
        modelssn = lambda data: sparse_ssn_iter(data, nspix, n_iter)
...
if __name__ == "__main__":
    ...
    #           cv2.imwrite(('superpixel/'+ str(i)+'crop.png'),crop_img)
            ...
            if (index == 0):
                mask[label==i] = [0,0,0]
            if (index == 3):
                mask[label==i] = [192, 192,192]
            if (index == 5):
                mask[label==i] = [147 , 97,41]
            if (index == 4):
                mask[label==i] = [255, 255, 255]
            if (index == 6):
                mask[label==i] = [0,128,0]
            if (index == 1):
                mask[label==i] = [5, 5, 255]
